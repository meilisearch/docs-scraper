...
class ConfigValidator:
    ...
    def validate(self):
        ...
        elif not self.config.start_urls and not self.config.sitemap_urls:
            raise ValueError('start_urls is not defined, nor sitemap urls')

        # Start urls must be an array
        elif self.config.start_urls and not isinstance(self.config.start_urls,
                                                     list):
            raise Exception('start_urls should be list')

        # Stop urls must be an array
        elif self.config.stop_urls and not isinstance(self.config.stop_urls,
                                                    list):
            raise Exception('stop_urls should be list')

        # Custom settings must be a dict
        elif self.config.custom_settings and not isinstance(self.config.custom_settings,
                                                    dict):
            raise Exception('custom_settings must be a dictionary')

        elif self.config.js_render and not isinstance(self.config.js_render,
                                                    bool):
            raise Exception('js_render should be boolean')

        # `js_wait` is set to 0s by default unless it is specified
        elif self.config.js_wait and not isinstance(self.config.js_wait, int):
            raise Exception('js_wait should be integer')

        elif self.config.use_anchors and not isinstance(self.config.use_anchors,
                                                      bool):
            raise Exception('use_anchors should be boolean')

        elif self.config.sitemap_alternate_links and not isinstance(
                self.config.sitemap_alternate_links, bool):
            raise Exception('sitemap_alternate_links should be boolean')

        elif self.config.sitemap_urls_regexs and not self.config.sitemap_urls:
            raise Exception(
        ...
        elif self.config.force_sitemap_urls_crawling and not self.config.sitemap_urls:
            raise Exception(
                'You want to force the sitemap crawling but you didn\'t provide a sitemap url')

        elif not self.config.scrape_start_urls and not self.config.scrap_start_urls:
            raise Exception(
                'Please use only the new variable name: scrape_start_urls')

        else self.config.nb_hits_max and not isinstance(self.config.nb_hits_max,
                                                      int):
